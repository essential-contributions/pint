...
impl Type {
    ...
    // - Unsized arrays where the element type is not bool, int, nor b256
    pub fn is_allowed_in_storage(&self, contract: &Contract) -> bool {
        match self {
            Type::UnsizedArray { ty, .. } => {
                // We only support dynamic arrays of these types for now
    ...
    pub fn storage_keys(
        ...
        match self {
            ...
            // `UnsizedArray` takes up a single storage slot that stores the length of the array.
    ...
    pub fn replace_type_expr(&mut self, old_expr: ExprKey, new_expr: ExprKey) {
        match self {
            ...
            Type::UnsizedArray { ty, .. } => ty.replace_type_expr(old_expr, new_expr),
