...
impl Type {
    ...
    pub fn get_union_variant_as_num(&self, contract: &Contract, tag: &String) -> Option<usize> {
    ...
    pub fn size(&self, handler: &Handler, contract: &Contract) -> Result<usize, ErrorEmitted> {
        match self {
            ...
            Self::Union { decl, .. } => {
                let Some(UnionDecl { variants, .. }) = contract.unions.get(*decl) else {
                    unreachable!("unknown decl key in union type")
                };
                    ...
                    (Some(lhs_custom_path), None) => {
                        if let Type::Union { decl, .. } = rhs_ty {
                            ...
                            contract
                                .unions
                                .get(*decl)
                                .map(|union| &union.name.name == lhs_custom_path)
                                .unwrap_or(false)
                        } else {
                            false
                        }
                    ...
                    (None, Some(rhs_custom_path)) => {
                        if let Type::Union { decl, .. } = lhs_ty {
                            ...
                            contract
                                .unions
                                .get(*decl)
                                .map(|union| &union.name.name == rhs_custom_path)
                                .unwrap_or(false)
                        } else {
                            false
                        }
