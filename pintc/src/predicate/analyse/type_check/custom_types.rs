        ...
        fn replace_alias_target(
            ...
            match target_ty {
                Type::Custom { name, .. } => {
                    if let Some(new_ty) = alias_map.get(name) {
                        *target_ty = new_ty.clone();
                        ...
                        todo!("unknown custom type for new-type target");
                    }
                }
                ...
                Type::Tuple { fields, .. } => {
                    modified = fields
                        .iter_mut()
                        .any(|(_, ty)| replace_alias_target(alias_map, ty));
                }
            ...
            match ty {
                Type::Custom { name, span } => {
                    if let Some(new_ty) = new_types.get(name) {
                        println!("LOWERING {name} to alias of {new_ty:?}");
                        *ty = Type::Alias {
                            name: name.clone(),
                        ...
                        println!("LOWERING {name} to union of {decl_key:?}");
                        *ty = Type::Union {
                            decl: *decl_key,
                            span: span.clone(),
                        }
