...
fn lower_storage_accesses_in_predicate(
    ...
    for expr in storage_accesses {
        ...
        let (addr, mutable, next_state, key, len_keys) =
...
> {
    ...
    match &expr.get(contract).clone() {
        Expr::UnaryOp {
            ...
            expr,
            ..
        } => get_base_storage_key(handler, expr, contract).map(
            |(addr, mutable, _, key, len_keys)| {
                (addr, mutable, true /* post-state */, key, len_keys)
            },
        ),
