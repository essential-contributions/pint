...
impl<'a> PredicateCompiler<'a> {
    ...
    fn compile_expr_key(&mut self, expr_key: predicate::ExprKey) -> Result<NodeKey, ErrorEmitted> {
        match expr_key.get(&self.contract) {
            ...
            Expr::UnaryOp { op, expr, span } => match op {
                ...
                UnaryOp::Not => self.compile_op(NOT, expr, None),
            ...
            Expr::BinaryOp { op, lhs, rhs, span } => {
                if let Some(asm_op) = Self::translate_op(op) {
                    self.compile_op(asm_op, lhs, Some(rhs))
                } else {
                    match op {
    ...
    fn compile_op(
        ...
        app_args.push(self.move_node_to(arg1_node_key, Location::Stack));

        if let Some(arg2_expr_key) = arg2 {
            let arg2_node_key = self.compile_expr_key(*arg2_expr_key)?;
            app_args.push(self.move_node_to(arg2_node_key, Location::Stack));
        }
        ...
        self.compile_app(applicand_node_key, app_args)
    }
    ...
    fn move_node_to(&mut self, src_node_key: NodeKey, loc: Location) -> NodeKey {
        todo!()
    }
}
