...
fn err() {
    ...
    check(
        &yurtc_command("").stderr,
        expect_test::expect![[r#"
            error: the following required arguments were not provided:
              <FILEPATH>

            Usage: yurtc <FILEPATH>
    ...
    check(
        &yurtc_command("missing.yrt").stderr,
        expect_test::expect![[r#"
            Error: couldn't read missing.yrt: No such file or directory (os error 2)
            Error: could not compile `missing.yrt` due to previous error
        "#]],
    );
    ...
    let output = yurtc_command(&format!("{} -o", input_file.path().to_str().unwrap(),));
    check(
        &output.stderr,
        expect_test::expect![[r#"
...
fn compile_errors() {
    ...
    let output = yurtc_command(input_file.path().to_str().unwrap());
...
fn explicit_output() {
    ...
    let output = yurtc_command(&format!(
        "{} -o {}",
...
fn solve() {
    ...
    let output = yurtc_command(&format!("{} --solve", input_file.path().to_str().unwrap(),));
