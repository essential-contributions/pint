#[cfg(test)]
use crate::lexer::*;

#[cfg(test)]
fn lex_one_success(src: &str) -> Token {
    // Tokenise src, assume success and that we produce a single token.

    let (toks, errs) = lex(src);
    assert!(errs.is_empty(), "Testing for success only.");
    assert_eq!(toks.len(), 1, "Testing for single token only.");
    toks[0].0.clone()
}

#[test]
fn newlines() {
    assert_eq!(lex_one_success("\\n"), Token::Newline);
    assert_eq!(lex_one_success(r"\n"), Token::Newline);
    assert_eq!(
        lex_one_success(
            r"
            "
        ),
        Token::Newline
    );
}

#[test]
fn comments() {
    assert_eq!(lex_one_success("//"), Token::Comment("//".to_string()));
    assert_eq!(lex_one_success("//\n"), Token::Comment("//\n".to_string()));
    assert_eq!(
        lex_one_success("// Hello"),
        Token::Comment("// Hello".to_string())
    );
    assert_eq!(
        lex_one_success("// Hello\n"),
        Token::Comment("// Hello\n".to_string())
    );
    assert_eq!(
        lex_one_success("// Hello\r"),
        Token::Comment("// Hello\r".to_string())
    );
    assert_eq!(
        lex_one_success("// Hello\r\n"),
        Token::Comment("// Hello\r\n".to_string())
    );
}

#[test]
fn control_tokens() {
    assert_eq!(lex_one_success(":"), Token::Colon);
    assert_eq!(lex_one_success("::"), Token::DoubleColon);
    assert_eq!(lex_one_success(";"), Token::Semi);
    assert_eq!(lex_one_success("="), Token::Eq);
    assert_eq!(lex_one_success(","), Token::Comma);
    assert_eq!(lex_one_success("{"), Token::BraceOpen);
    assert_eq!(lex_one_success("}"), Token::BraceClose);
    assert_eq!(lex_one_success("("), Token::ParenOpen);
    assert_eq!(lex_one_success(")"), Token::ParenClose);
    assert_eq!(lex_one_success("="), Token::Eq);
    assert_eq!(lex_one_success("->"), Token::Arrow);
    assert_eq!(lex_one_success("=>"), Token::HeavyArrow);
    assert_eq!(lex_one_success("|"), Token::Pipe);
    assert_eq!(lex_one_success("."), Token::Dot);
    assert_eq!(lex_one_success(".."), Token::TwoDots);
}

#[test]
fn primitives() {
    assert_eq!(
        lex_one_success("real"),
        Token::Primitive("real".to_string())
    );
    assert_eq!(lex_one_success("int"), Token::Primitive("int".to_string()));
    assert_eq!(
        lex_one_success("string"),
        Token::Primitive("string".to_string())
    );
    assert_eq!(
        lex_one_success("bool"),
        Token::Primitive("bool".to_string())
    );
}

#[test]
fn idents() {
    assert_eq!(
        lex_one_success("identifier"),
        Token::Ident("identifier".to_string())
    );
    assert_eq!(
        lex_one_success("_privateVar"),
        Token::Ident("_privateVar".to_string())
    );
    assert_eq!(
        lex_one_success("var123"),
        Token::Ident("var123".to_string())
    );
}

#[test]
fn bool_literals() {
    assert_eq!(lex_one_success("true"), Token::Literal("true".to_string()));
    assert_eq!(
        lex_one_success("false"),
        Token::Literal("false".to_string())
    );
}

#[test]
fn number_literals() {
    assert_eq!(lex_one_success("42"), Token::Literal("42".to_string()));
    assert_eq!(lex_one_success("3.14"), Token::Literal("3.14".to_string()));
    assert_eq!(
        lex_one_success("5.67E+3"),
        Token::Literal("5.67E+3".to_string())
    );
    assert_eq!(
        lex_one_success("1.23e-4"),
        Token::Literal("1.23e-4".to_string())
    );
    assert_eq!(
        lex_one_success("0x1A3F"),
        Token::Literal("0x1A3F".to_string())
    );
    assert_eq!(
        lex_one_success("0b1010"),
        Token::Literal("0b1010".to_string())
    );
}

#[test]
fn string_literals() {
    assert_eq!(
        lex_one_success(r#""Hello""#),
        Token::Literal(r#""Hello""#.to_string())
    );
    assert_eq!(
        lex_one_success(r#""This is a \"quote\".""#),
        Token::Literal(r#""This is a \"quote\".""#.to_string())
    );
    assert_eq!(
        lex_one_success(r#""New\nLine""#),
        Token::Literal(r#""New\nLine""#.to_string())
    );
}

#[test]
fn solve_tokens() {
    assert_eq!(lex_one_success("solve"), Token::Solve);
    assert_eq!(
        lex_one_success("maximize"),
        Token::Directive("maximize".to_string())
    );
    assert_eq!(
        lex_one_success("minimize"),
        Token::Directive("minimize".to_string())
    );
    assert_eq!(
        lex_one_success("satisfy"),
        Token::Directive("satisfy".to_string())
    );
}

#[test]
fn keywords() {
    assert_eq!(lex_one_success("let"), Token::Let);
    assert_eq!(lex_one_success("fn"), Token::Fn);
    assert_eq!(lex_one_success("in"), Token::In);
    assert_eq!(lex_one_success("if"), Token::If);
    assert_eq!(lex_one_success("else"), Token::Else);
    assert_eq!(lex_one_success("cond"), Token::Cond);
    assert_eq!(lex_one_success("enum"), Token::Enum);
    assert_eq!(lex_one_success("type"), Token::Type);
    assert_eq!(lex_one_success("use"), Token::Use);
    assert_eq!(lex_one_success("as"), Token::As);
    assert_eq!(lex_one_success("constraint"), Token::Constraint);
    assert_eq!(lex_one_success("state"), Token::State);
}

#[test]
fn operators() {
    assert_eq!(lex_one_success("!"), Token::Bang);
    assert_eq!(lex_one_success("+"), Token::Plus);
    assert_eq!(lex_one_success("-"), Token::Minus);
    assert_eq!(lex_one_success("*"), Token::BinaryOp("*".to_string()));
    assert_eq!(lex_one_success("/"), Token::BinaryOp("/".to_string()));
    assert_eq!(lex_one_success("%"), Token::BinaryOp("%".to_string()));
    assert_eq!(lex_one_success(">"), Token::BinaryOp(">".to_string()));
    assert_eq!(lex_one_success("<"), Token::BinaryOp("<".to_string()));
    assert_eq!(lex_one_success("<="), Token::BinaryOp("<=".to_string()));
    assert_eq!(lex_one_success(">="), Token::BinaryOp(">=".to_string()));
    assert_eq!(lex_one_success("=="), Token::BinaryOp("==".to_string()));
    assert_eq!(lex_one_success("!="), Token::BinaryOp("!=".to_string()));
    assert_eq!(lex_one_success("&&"), Token::BinaryOp("&&".to_string()));
    assert_eq!(lex_one_success("||"), Token::BinaryOp("||".to_string()));
}
