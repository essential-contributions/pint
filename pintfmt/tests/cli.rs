...
#[cfg(test)]
fn pintfmt_command(args: &str) -> Output {
    let output = test_bin::get_test_bin("pintfmt")
        .args(args.split_ascii_whitespace().collect::<Vec<_>>())
        .output()
        .expect("failed to start pintfmt");
...
fn err() {
    ...
    check(
        &pintfmt_command("").stderr,
        expect_test::expect![[r#"
            error: the following required arguments were not provided:
              <FILEPATHS>...

            Usage: pintfmt <FILEPATHS>...
    ...
    check(
        &pintfmt_command("missing.pnt").stderr,
        expect_test::expect![[r#"
...
fn compile_errors() {
    ...
    let output = pintfmt_command(input_file.path().to_str().unwrap());
...
fn multiple_file() {
    ...
    let output = pintfmt_command(&format!(
        "{} {}",
